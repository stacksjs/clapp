...
export function autocomplete<Value>(opts: AutocompleteOptions<Value>) {
    ...
    render() {
      ...
      switch (this.state) {
        ...
        default: {
          ...
          const matches = this.filteredOptions.length !== options.length ? color.dim(` (${this.filteredOptions.length} match${this.filteredOptions.length === 1 ? '' : 'es'})`) : ''
          ...
          const displayOptions = this.filteredOptions.length === 0
            ...
            : limitOptions({
                ...
                style: (option, active) => {
                  ...
                  const hint = option.hint && option.value === this.focusedValue ? color.dim(` (${option.hint})`) : ''

                  return active ? `${color.green(S_RADIO_ACTIVE)} ${label}${hint}` : `${color.dim(S_RADIO_INACTIVE)} ${color.dim(label)}${hint}`
                },
          ...
          // No matches message
          const noResults = this.filteredOptions.length === 0 && userInput ? [`${color.cyan(S_BAR)}  ${color.yellow('No matches found')}`] : []

          const validationError = this.state === 'error' ? [`${color.yellow(S_BAR)}  ${color.yellow(this.error)}`] : []
 ...
 */
export function autocompleteMultiselect<Value>(opts: AutocompleteMultiSelectOptions<Value>) {
  const formatOption = (
    option: Option<Value>,
    active: boolean,
    ...
    const hint = option.hint && focusedValue !== undefined && option.value === focusedValue ? color.dim(` (${option.hint})`) : ''
  ...
  const prompt = new AutocompletePrompt<Option<Value>>({
    ...
    render() {
      ...
      const searchText = this.isNavigating || showPlaceholder ? color.dim(showPlaceholder ? placeholder : userInput) : this.userInputWithCursor
      ...
      switch (this.state) {
        ...
        default: {
          ...
          const errorMessage = this.state === 'error' ? [`${color.cyan(S_BAR)}  ${color.yellow(this.error)}`] : []
